...
func TestStartHandler(t *testing.T) {
	t.Run("with time parameter", func(t *testing.T) {
		...
		r := router.SetupRouter(zap.NewNop())
		timeStr := "2023-01-01T10:00:00"
		reqBody, _ := json.Marshal(handlers.TimeRequest{Time: &timeStr})
	...
	t.Run("without time parameter", func(t *testing.T) {
		...
		r := router.SetupRouter(zap.NewNop())
		reqBody, _ := json.Marshal(handlers.TimeRequest{Time: nil})
		req, err := http.NewRequest("POST", "/start", bytes.NewBuffer(reqBody))
		if err != nil {
			t.Fatal(err)
		}
...
func TestStopHandler(t *testing.T) {
	t.Run("with time parameter", func(t *testing.T) {
		...
		startReqBody, _ := json.Marshal(handlers.TimeRequest{Time: &startTimeStr})
